...
var (
	...
	testBitStrings     = make([]*bitstring.BitString, len(testStrings))
	templateBitStrings = make([]*bitstring.BitString, len(templateStrings))
)
...
func TestMain(m *testing.M) {
	...
	for i, s := range testStrings {
		if bs, err := bitstring.BitStringFromString(s); err != nil {
			os.Exit(1)
		} else {
			testBitStrings[i] = bs
		}
